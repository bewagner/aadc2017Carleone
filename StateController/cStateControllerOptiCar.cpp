...
ADTF_FILTER_PLUGIN("State Controller OptiCar", OID_ADTF_STATE_CONTOLLERTWO, cStateControllerOptiCar);
...
tResult cStateControllerOptiCar::Init(tInitStage eStage, __exception)
{
	...
	if (eStage == StageFirst)    
	{
		...
		// Output pin of Maneuver Code
		RETURN_IF_FAILED(m_oCurrentManeuver.Create("Maneuver_Code", pTypeInt32SignalValue, static_cast<IPinEventSink*> (this)));
		RETURN_IF_FAILED(RegisterPin(&m_oCurrentManeuver));
		RETURN_IF_FAILED(pTypeInt32SignalValue->GetInterface(IID_ADTF_MEDIA_TYPE_DESCRIPTION, (tVoid**)&m_pDescriptionInt32));
		
		// Output pin for current parking space ID
		RETURN_IF_FAILED(m_ParkingSpaceIDOutputPin.Create("CurrentParkingSpaceID", pTypeInt32SignalValue, static_cast<IPinEventSink*> (this)));
		RETURN_IF_FAILED(RegisterPin(&m_ParkingSpaceIDOutputPin));
		RETURN_IF_FAILED(pTypeInt32SignalValue->GetInterface(IID_ADTF_MEDIA_TYPE_DESCRIPTION, (tVoid**)&m_pDescriptionInt32));
	...
	{      
		...
		else if (pSource == &m_StateIncrementManeuverInputPin)
		{
			...
			if (bValue){ 
				if(m_filterProperties.debugOutput) { 
					LOG_INFO(cString::Format("SC: Received incrementManeuver signal") ); 
				}
				incrementManeuverID();
				transmitManeuverEntryToDM();
			}
		}
		...
		else if (pSource == &m_JuryStructInputPin) 
		{
			...
			case action_GETREADY:
				if(m_filterProperties.debugOutput) { 
					LOG_INFO(cString::Format("State Controller: Received: Request Ready with maneuver ID %d",i16entry)); 
				}
			...
			case action_START:
				if(m_filterProperties.debugOutput) { 
					LOG_INFO(cString::Format("State Controller: Received: Run with maneuver ID %d",i16entry)); 
				}
				...
				transmitStateToDM(tTrue);
				break;
			case action_STOP:
				if(m_filterProperties.debugOutput) { 
					LOG_INFO(cString::Format("State Controller: Received: Stop with maneuver ID %d",i16entry)); 
				}
				changeState(stateCar_STARTUP);
				transmitStateToDM(tFalse);
				break;					
			}
			transmitManeuverEntryToDM();
...
tResult cStateControllerOptiCar::TransmitParkingSpaceID(tInt16 parkingSpaceID)
{
	__synchronized_obj(CritParkingIDOut);
	...
	{
		__adtf_sample_write_lock_mediadescription(m_pDescriptionInt32, pMediaSample, pCoderOutput);

		if (!m_bIDsParkingIDSet)
		{
			pCoderOutput->GetID("intValue",m_szIDParkingIDBuffer);
			m_bIDsParkingIDSet = tTrue;
		}
		pCoderOutput->Set(m_szIDParkingIDBuffer, (tVoid*)&parkingSpaceID);
	}
	pMediaSample->SetTime(_clock->GetStreamTime());
PRINT1("ParkingspaceID in StateControll: %i",parkingSpaceID);
	RETURN_IF_FAILED(m_ParkingSpaceIDOutputPin.Transmit(pMediaSample));
	RETURN_NOERROR;
}

tResult cStateControllerOptiCar::SendState(stateCar state, tInt16 i16ManeuverEntry)
{            
	__synchronized_obj(CritSendStateOut);
	...
	if(m_bDebugModeEnabled)  
	{
		...
		case stateCar_ERROR:
			if(m_filterProperties.debugOutput) { 
				LOG_INFO(cString::Format("State Controller Module: Send state: ERROR, Maneuver ID %d",i16ManeuverEntry)); 
			}
			break;
		case stateCar_READY:
			if(m_filterProperties.debugOutput) { 
				LOG_INFO(cString::Format("State Controller Module: Send state: READY, Maneuver ID %d",i16ManeuverEntry)); 
			}
			break;
		case stateCar_RUNNING:
			if(m_filterProperties.debugOutput) { 
				LOG_INFO(cString::Format("State Controller Module: Send state: RUNNING, Maneuver ID %d",i16ManeuverEntry)); 
			}
			break;
		case stateCar_COMPLETE:
			if(m_filterProperties.debugOutput) { 
				LOG_INFO(cString::Format("State Controller Module: Send state: COMPLETE, Maneuver ID %d",i16ManeuverEntry));                    
			}
			break;
...
tResult cStateControllerOptiCar::incrementManeuverID()
{
	...
	if(m_filterProperties.debugOutput) { 
		LOG_INFO(adtf_util::cString::Format("Increment Manevuer ID: Sectionindex is %d, Maneuverindex is %d, ID is %d",m_i16SectionListIndex,m_i16ManeuverListIndex,m_i16CurrentManeuverID)); 
	}
...
tResult cStateControllerOptiCar::resetSection()
{
	...
	if(m_filterProperties.debugOutput) { 
		LOG_INFO(adtf_util::cString::Format("Reset section: Sectionindex is %d, Maneuverindex is %d, ID is %d",m_i16SectionListIndex,m_i16ManeuverListIndex,m_i16CurrentManeuverID)); 
	}
	...
	for(unsigned int i = 0; i < m_sectorList.size(); i++)
	{
		for(unsigned int j = 0; j < m_sectorList[i].maneuverList.size(); j++)
		{
			...
			{            
				...
				if(m_filterProperties.debugOutput) { 
					LOG_INFO(adtf_util::cString::Format("Sectionindex is %d, Maneuverindex is %d, ID is %d",m_i16SectionListIndex,m_i16ManeuverListIndex,m_i16CurrentManeuverID)); 
				}
				break;
			}
		}
...
tResult cStateControllerOptiCar::transmitStateToDM(tBool run_value){

	__synchronized_obj(CritStatetoDMOut);
	cObjectPtr<IMediaSample> pMediaSample;
	...
	RETURN_IF_FAILED(m_oStartDM.Transmit(pMediaSample));

	RETURN_NOERROR;
}


vector<string> cStateControllerOptiCar::ParseParkingString(cString input){
	string manueverAction (input);
	std::istringstream iss(manueverAction);
...
tResult cStateControllerOptiCar::transmitManeuverEntryToDM(){
	...
	if (m_i16ManeuverListIndex!=-1 && m_i16SectionListIndex!=-1)
	{
		...
		else
		{
			...
			vector<string> maneuverAndID = ParseParkingString(man.action);
			if(maneuverAndID.size() > 1) {
				TransmitParkingSpaceID(atoi(maneuverAndID[1].c_str()));
				man.action = maneuverAndID[0].c_str();
			}
						


			tInt32 action_id	= getManeuverCode(man.action);
		...
		tUInt32 ui32TimeStamp 	= 0;
		{
			...
			// set the id
			pCoder->GetID("intValue", m_szIDManeuverCodeOutput);
			pCoder->GetID("ui32Timestamp", m_szIDTimestampManeuverCodeOutput);

			// set value from sample
			pCoder->Set(m_szIDManeuverCodeOutput, (tVoid*)&intValue);
			pCoder->Set(m_szIDTimestampManeuverCodeOutput, (tVoid*)&(ui32TimeStamp));
		}
		//cout << "ActionID: " << intValue << endl;
		pMediaSample->SetTime(_clock->GetStreamTime());

		m_oCurrentManeuver.Transmit(pMediaSample);
 ...
 * 6	- pull_out_right
 */
tInt32 cStateControllerOptiCar::getManeuverCode(cString action){

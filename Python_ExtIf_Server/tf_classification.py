...
config = {
    ...
    "orientation_classification_model": 'model/orientation_classification_graph.pb'
}
"""
...
def distance(ymax):
    #return (-1) / (.803) * np.log((ymax - .1806)/.53361606)
    return (np.tan((ymax-321.2250165)/204.41201081)+25.09756939)/(-660.29893851)
...
def classify_image(image_data):
    ...
    for i in range(config['number_of_top_predictions']):
        ...
        kidThreshold = float(0.175)
        pictureHeight = float(image.shape[0])
        cutOutRatio = float(ymax - ymin) / pictureHeight
        
        if distance(ymax) > 1.5:
            result_list.append(TDataResult("dummy",confidence,ymin,xmin,ymax,xmax,"None",False))            
        
        if ymin > kidThreshold and label in ["person", "teddy bear", "toothbrush", "fire hydrant" ,"skateboard"]:
        ...
        # If we found a person we additionally have to classify its orientation
            ...
            result_list.append(TDataResult(label,confidence,ymin,xmin,ymax,xmax,orientation,False))            
        else:
